...
def action1(input_path_root, output_path_root):
    print(f"導入路徑: {input_path_root}, 導出路徑: {output_path_root}")
    print('action1 execute')
...
def initialize_ui(root):
    ...
    # 執行按鈕
    execute_button = tk.Button(root, text="執行", command= lambda: action1(*get_current_path())) # 主體用執行程序, 接收傳參函數返回值
    # execute_button = tk.Button(root, text="執行", command=lambda: action1(get_paths()[0], get_paths()[1])) # 等價
